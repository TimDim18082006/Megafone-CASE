...
def dict_statistic(data_df=None, stat=None, perc_1=None, perc_2=None, moda=False, n=1, nan_info=False):
    ...
    if moda:
        ...
        res_df["tt"] = res_df.apply(lambda x: mode_text(x.tt), axis=1)  # составное текстовое имя
...
def data_describe(data_df, style=1):
    ...
    metric_dic = {'count': 'A00', 'mean': 'B01', 'std': 'B02', 'min': 'C01', '25%': 'C02', '50%': 'C03', '75%': 'C04',
                  'max': 'C05', 'variance': 'B03', 'IQR': 'C07', 'kurtosis': 'D01', 'skew': 'D02', 'NaN_count': 'A01',
                  'upper_limit': 'C06', 'lower_limit': 'C00'}
    ...
    df_IQR: DataFrame = pd.DataFrame.from_dict(dict_statistic(data_df, np.percentile, 25, 75)[0], orient='index')
    df_75: DataFrame = pd.DataFrame.from_dict(dict_statistic(data_df, np.percentile, 75)[0], orient='index')
    df_25: DataFrame = pd.DataFrame.from_dict(dict_statistic(data_df, np.percentile, 25)[0], orient='index')
...
def class_describe(data_y=pd.Series(), data_x=pd.Series(), df=pd.DataFrame(), name_y='Actual', name_x='Predicted',
                   name_title='Confusion matrix'):
    """
    :param data_y:      пандас серия
    :param data_x:      пандас серия
    :param df:          датафрейм уже сформированного отчета (если да то первые параметры игнор)
    ...
    sns.heatmap(((df / df.iloc[-1, :] * 100).iloc[:, : -1]).round(1), cmap='viridis', annot=True, fmt='g',
                linewidths=.5)
    ax.xaxis.set_label_position("top")
    plt.tight_layout()
